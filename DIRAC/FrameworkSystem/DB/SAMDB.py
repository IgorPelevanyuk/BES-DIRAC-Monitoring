...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ REVISION LINE ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    def getState(self):
        ...
        sqlSelect = "SELECT s.name, t.name, r.state, UNIX_TIMESTAMP(UTC_TIMESTAMP())-UNIX_TIMESTAMP(r.last_update) as last, r.description FROM States st, Sites s, Tests t, Results r WHERE s.site_id=r.site_id AND r.result_id=st.result_id"
        result = self._query( sqlSelect )
        print result['Value']
        return result
